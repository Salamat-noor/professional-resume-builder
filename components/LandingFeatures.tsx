const features = [
  { icon: 'ri-sparkling-2-line', color: 'bg-indigo-50 text-indigo-600', title: 'AI Writing Assistant', desc: 'Rewrite bullets with measurable impact, generate summaries, and improve clarity with one click.' },
    ...
    <section className="py-24 bg-white">
      <div className="max-w-7xl mx-auto px-6">
        <div className="text-center max-w-2xl mx-auto mb-14">
          ...
          <h2 className="text-4xl font-bold text-gray-900 mb-4">Professional tools for modern job seekers</h2>
          <p className="text-lg text-gray-600">From first draft to final offer — ClarityCV powers every step of your job search.</p>
        </div>
        <div className="grid grid-cols-3 gap-6">
          {features.map(f => (
            <div key={f.title} className="bg-white rounded-2xl border border-gray-100 p-7 hover:shadow-xl hover:border-indigo-100 transition-all group cursor-default">
              ...
              <span className="text-sm text-indigo-600 font-medium group-hover:gap-2 flex items-center gap-1 transition-all">Learn more <i className="ri-arrow-right-line"></i></span>
            </div>
          ))}
        </div>
      </div>
