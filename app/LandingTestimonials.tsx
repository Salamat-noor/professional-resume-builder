    ...
    <section className="py-24 bg-gradient-to-b from-slate-50 to-white">
      <div className="max-w-7xl mx-auto px-6">
        <div className="text-center mb-14">
          <p className="text-xs font-semibold text-indigo-600 uppercase tracking-widest mb-3">Success Stories</p>
          <h2 className="text-4xl font-bold text-gray-900">Hired. Promoted. Thriving.</h2>
        </div>
        <div className="bg-white rounded-3xl shadow-lg border border-gray-100 overflow-hidden">
          <div className="grid grid-cols-5">
            <div className="col-span-2 bg-gradient-to-br from-indigo-600 to-blue-600 p-10 flex flex-col justify-between">
              <div>
                ...
                <p className="text-indigo-200 text-sm mb-4">{t.role}</p>
                <div className="flex gap-1">{[...Array(t.rating)].map((_,i) => <i key={i} className="ri-star-fill text-yellow-400 text-sm"></i>)}</div>
              </div>
              <div className="flex gap-3 mt-8">
                {testimonials.map((_,i) => <button key={i} onClick={() => setActive(i)} className={`w-2.5 h-2.5 rounded-full transition-all cursor-pointer ${i === active ? 'bg-white scale-125' : 'bg-white/40'}`} />)}
              </div>
            </div>
        ...
        <div className="grid grid-cols-3 gap-6 mt-8">
          {[{n:'50K+',l:'Resumes Created'},{n:'94%',l:'Interview Rate Increase'},{n:'3 Weeks',l:'Avg. Time to Offer'}].map(s => (
            <div key={s.l} className="text-center p-6 bg-white rounded-2xl border border-gray-100 shadow-sm">
              <p className="text-3xl font-bold text-indigo-600 mb-1">{s.n}</p>
